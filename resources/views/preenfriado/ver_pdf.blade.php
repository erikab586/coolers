...
<head>
    <meta charset="UTF-8">
    <title>Preenfriado - Tarima {{ $preenfriado->tarima->codigo }}</title>
    <style>
        body {
...
<body>
    ...
    <table class="details-table">
            ...
            <tbody>
                @forelse($preenfriado->detallesPreenfriado as $detalle)
                   @php
                ...
                <tr>
                    <td>{{ $detalle->tarimaDetarec->detalle->recepcion->folio }}</td>
                    <td>{{ $detalle->tarimaDetarec->detalle->fruta->nombrefruta ?? 'N/A' }}</td>
                    <td>{{ $detalle->tarimaDetarec->detalle->variedad->tipofruta ?? 'N/A' }}</td>
                    <td>{{ $detalle->tarimaDetarec->detalle->presentacion->nombrepresentacion ?? 'N/A' }}</td>
    ...
    <div style="margin-top: 30px;">
        <h3 style="background-color: #f0f0f0; padding: 8px; border-left: 4px solid #ffc107;">
            ...
            <tr>
                <td style="width: 50%; text-align: center; padding: 10px; vertical-align: top;">
                    <div style="border: 1px solid #ddd; padding: 10px;">
                        <img src="{{ $preenfriado->firma_responsable1 }}" style="max-width: 200px; height: 80px;">
                        <p style="margin-top: 10px; margin-bottom: 5px;"><strong>{{ $preenfriado->nombre_responsable1 }}</strong></p>
                        <p style="margin: 0; font-size: 9pt; color: #666;">Responsable 1</p>
                    </div>
                </td>
                <td style="width: 50%; text-align: center; padding: 10px; vertical-align: top;">
                    <div style="border: 1px solid #ddd; padding: 10px;">
                        <img src="{{ $preenfriado->firma_responsable2 }}" style="max-width: 200px; height: 80px;">
                        <p style="margin-top: 10px; margin-bottom: 5px;"><strong>{{ $preenfriado->nombre_responsable2 }}</strong></p>
    ...
    <div style="margin-top: 15px; padding: 10px; background-color: #fffacd; border: 1px solid #ddd;">
        <strong>NOTA:</strong> {{ $preenfriado->nota_firmas }}
    </div>
    @endif
    @endif
