...
@section('content')
<div class="pagetitle">
  <h1>Formulario de Embarque </h1>
  <nav>
    <ol class="breadcrumb">
      <li class="breadcrumb-item">Embarcaciones</li>
    ...
    <div class="col-lg-12">
      <div class="card">
        ...
        @if (session('success'))
            ...
            @endpush
        @endif
            <form class="row g-3" action="{{ route('embarcacion.guardar') }}" method="POST">
                @csrf
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control @error('user') is-invalid @enderror" value="{{ auth()->user()->name }} {{ auth()->user()->apellidos }}" readonly>
                                <input type="hidden" name="id_usuario" value="{{ auth()->user()->id }}">
                                @error('user')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <input type="hidden" class="form-control" name="firma_usuario" value="No Aplica" >
                            </div>
                                {{-- Nombre Cliente --}}
                            <div class="col-md-4">
                                <label class="form-label">Nombre Cliente</label>
                                <input type="text" class="form-control @error('nombre_responsblecliente') is-invalid @enderror" name="nombre_responsblecliente" value="{{ old('nombre_responsblecliente') }}">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control @error('apellido_responsablecliente') is-invalid @enderror" name="apellido_responsablecliente" >
                                @error('apellido_responsablecliente')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <input type="hidden" class="form-control" name="firma_cliente"  >
                        </div>
                        <div class="row">
                            {{-- Nombre Chofer --}}
                            <div class="col-md-4">
                                <label class="form-label">Nombre Chofer</label>
                                <input type="text" class="form-control @error('nombre_responsblechofer') is-invalid @enderror" name="nombre_responsblechofer" value="{{ old('nombre_responsblechofer') }}">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control @error('apellido_responsablechofer') is-invalid @enderror" name="apellido_responsablechofer" value="{{ old('apellido_responsablechofer') }}">
                           ...
                           <input type="hidden" class="form-control" name="firma_chofer"  value="No Aplica">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control @error('prod_ultimacarga') is-invalid @enderror"name="prod_ultimacarga"> @error('prod_ultimacarga')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="col-lg-8">
                        ...
                        <table class="table table-bordered" id="tabla-espreenfrio">
                            ...
                            <tbody id="detalles-body">
                                ...
                                <tr>
                                    <td>
                                        <input type="hidden" name="idconservacion[]" value="{{ $conservaciones->id }}">
                                       <input type="hidden" name="iddetalletarima[]" value="{{ $data->tarimaDetarec->id }}">
                                        <input type="text" class="form-control" name="idtarima[]" value="{{ $conservaciones->idtarima ?? 'N/A' }}">
                                    </td>
                                    <td>
                    ...
                    <div class="col-lg-4">
                        ...
                        @php
                            // Agrupar y sumar cantidades por combinación de fruta, variedad y presentación
                            $totales = [];
                            foreach($conservaciones->detallesConservacion as $data) {
                            ...
                            // Convertir a array indexado y limitar a 6 elementos
                            $totalesArray = array_values($totales);
                            $totalesArray = array_slice($totalesArray, 0, 6);
                        @endphp
                        
                        @for($i = 0; $i < 6; $i++)
                            <div class="col-md-12">
                                <label class="form-label">Total {{ $i + 1 }}</label>
                                @if(isset($totalesArray[$i]))
                                    <input type="text" 
                                        class="form-control @error('total' . ($i + 1)) is-invalid @enderror" 
                                        name="total{{ $i + 1 }}" 
                                        value="{{ old('total' . ($i + 1), $totalesArray[$i]['fruta'] . ' - ' . $totalesArray[$i]['variedad'] . ' - ' . $totalesArray[$i]['presentacion'] . ': ' . $totalesArray[$i]['cantidad']) }}" 
                                        readonly>
                                @else
                                    <input type="text" 
                                        class="form-control @error('total' . ($i + 1)) is-invalid @enderror" 
                                        name="total{{ $i + 1 }}" 
                                        value="{{ old('total' . ($i + 1), '0') }}" 
                                        readonly>
                                @endif
                                @error('total' . ($i + 1))
                                    <div class="invalid-feedback">{{ $message }}</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        radios.forEach(radio => {
            radio.addEventListener('click', function() {
                ...
                radios.forEach(r => r.previousChecked = r.checked);
            });
        });
         const alert = document.querySelector('.alert.alert-danger.alert-dismissible');

        if (alert) {
            setTimeout(function () {
                // Si usas Bootstrap 5 y tienes JS de Bootstrap cargado:
